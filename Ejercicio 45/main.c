...
typedef struct repuestos_str41{
   long partNumber;
   struct repuestos_str41 *lazo;
};
...
int main()
{
            ...
            if(aux_ex)
            {
                ...
                if(p_ex == NULL)
                {
                    ...
                    if(strcmp(r_ex->repuesto->modelo,aux_ex->repuesto->modelo) > 0)
                    {
                        ...
                        while(r_ex->lazo != NULL && (strcmp(r_ex->lazo->repuesto->modelo,aux_ex->repuesto->modelo) < 0 ))
                        {
                            r_ex = r_ex->lazo;
                        }
                        if(r_ex == u_ex)
    ...
    struct repuestos_str41 *p_41, *aux_41, *u_41;
    p_41 = u_41 = NULL;
    aux_ex = p_ex;
    while(aux_ex)
    {
        //Suponiendo que el número de parte y el modelo constan de una secuencia de 6 dígitos
        long model = charToInt(aux_ex->repuesto->modelo[5])*100000 +
                     charToInt(aux_ex->repuesto->modelo[4])*10000 +
                     charToInt(aux_ex->repuesto->modelo[3])*1000 +
                     charToInt(aux_ex->repuesto->modelo[2])*100 +
                     charToInt(aux_ex->repuesto->modelo[1])*10 +
                     charToInt(aux_ex->repuesto->modelo[0]);
        int max = aux_ex->cantidad, i = 0;
        repuestos_t41 stock;
        FILE *f, *f2;
        ...
        if(f != NULL)
        {
            ...
            while(!feof(f))
            {
                if(stock.partNumber == model && i<max)
                {
                    i++;
                } else {
                    f2 = fopen("stock_nuevo.dat","ab");
                    fwrite(&stock,sizeof(stock),1,f2);
                    fclose(f2);
                }
                fread(&stock,sizeof(stock),1,f);
            }
        ...
        if(i != max)
        {
            aux_41 = (struct repuestos_str41 *) malloc(sizeof(struct repuestos_str41));
            if(aux_41)
            {
                aux_41->partNumber = model;
                if(p_41 == NULL)
                {
                    p_41 = aux_41;
                    u_41 = aux_41;
        ...
        fclose(f);
        f2 = fopen("stock_nuevo.dat","rb");
        if(f2 != NULL)
        {
            fread(&stock,sizeof(stock),1,f2);
            while(!feof(f2))
            {
